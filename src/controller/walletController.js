...
// ==============================
    ...
    sendTransaction:async (req, res) => {
        ...
        try {
          ...
          res.status(200).json({ msg: "success",sendTransaction })
        } catch (error) {
...
updateDeviceToken:async (req, res) => {
    const {address, deviceToken } = req.body;
  
    try {
      const updateDeviceToken = await walletService.updateDeviceToken(req.body);
      res.status(200).json({ msg: "success",updateDeviceToken })
    } catch (error) {
      res.status(500).send('Error fetching updateDeviceToken');
    }
  },
